...
test('supports inherit', t => {
  t.is(getClassName({ className: 'w' }, 'inherit'), 'al-w_inherit');
});

test('supports initial', t => {
  t.is(getClassName({ className: 'w' }, 'initial'), 'al-w_initial');
});

test('supports unset', t => {
  t.is(getClassName({ className: 'w' }, 'unset'), 'al-w_unset');
});

test('safeValue does not transform value', t => {
  const result = getClassName({ className: 'w', safeValue: true }, '50.5%');
  t.is(result, 'al-w_50.5%');
});
